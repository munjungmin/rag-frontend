...
function ProjectPage({params}: ProjectPageProps) {
    ...
    useEffect(() => {
        ...
        const loadAllData = async () => {
            ...
            try {
                ...
                // 4개의 API Call을 병렬로 수행(의존성이 서로 없을 경우 가능), 하나라도 실패하면 전부 실패 처리
    ...
    // short polling - celery가 작업하는 문서의 진행 과정을 계속 가져옴 
    useEffect(() => {
        const hasProcessingDocuments = data.documents.some(
            (doc) => 
                doc.processing_status && 
                !["completed", "failed"].includes(doc.processing_status)   // 처리가 끝난건 polilng안함 
        );
  ...
  return (
    <>
        <div>
            <div className="flex h-screen bg-[#ffffff] gap-4 p-4"> 
                <ConversationsList 
        ...
        { selectedDocument && (
            <FileDetailsModal     // selectedDocument가 null이면 안보여주고 null이 아니면 보여줌 
